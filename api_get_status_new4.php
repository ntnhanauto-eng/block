...
while ($row = mysqli_fetch_assoc($rooms_query)) {
    ...
    if (!empty($token) && isset($devices[$current_room_id])) {
        ...
        if ($tuya_door_state !== $previous_state) {
            ...
            // 1. Nếu phòng đang TRỐNG mà CỬA MỞ -> Báo động ngay lập tức
        ...
        // =========================================================================
        // 🔥 ĐÃ CẤU HÌNH TRỌN VẸN: LOGIC PHÂN TẦNG VÀ ĐẾM NHẮC NHỞ ĐỊNH KỲ MỖI 2 PHÚT (BẰNG PHP TRỰC TIẾP)
        // =========================================================================
        if ($tuya_door_state === 'Mở' && ($row['status'] === 've_sinh' || $row['status'] === 've_sink')) {
            
            // Tìm mốc thời gian Lễ tân bấm Check-out chuyển phòng sang trạng thái Vệ sinh gần nhất
            $checkout_log_q = mysqli_query($conn, "
                SELECT event_time FROM room_logs 
                WHERE room_id = $current_room_id 
                  AND (details LIKE '%thành (Đang Vệ Sinh)%' OR details LIKE '%thành (ve_sinh)%' OR details LIKE '%vệ sinh%') 
                ORDER BY id DESC LIMIT 1
            ");
            $checkout_log = mysqli_fetch_assoc($checkout_log_q);
            $checkout_time = $checkout_log['event_time'] ?? date('Y-m-d H:i:s', strtotime('-1 hour'));
            
            // Kiểm tra xem Nhân viên buồng phòng đã bấm "BẮT ĐẦU DỌN PHÒNG" hay chưa
            $cleaner_start_q = mysqli_query($conn, "
            ...
            if (!$has_started_cleaning) {
                // Tính số phút trôi qua kể từ lúc Lễ tân thực hiện Check-out phòng
                $time_passed_minutes = (time() - strtotime($checkout_time)) / 60;

                if ($time_passed_minutes > 20) {
                    // Cấu hình chuẩn nội dung nhật ký theo yêu cầu
                    $alert_details = "Chưa bắt đầu dọn vệ sinh nhưng cửa mở bất thường";
                    
                    // Lấy log cảnh báo gần đây nhất của phòng này để làm cột mốc đối soát
                    $get_last_warning_q = mysqli_query($conn, "
                        SELECT event_time FROM room_logs 
                        WHERE room_id = $current_room_id 
                          AND details = '$alert_details' 
                        ORDER BY id DESC LIMIT 1
                    ");
                    $last_warning = mysqli_fetch_assoc($get_last_warning_q);

                    $allow_insert = false;
                    if (!$last_warning) {
                        // Nếu chưa từng có log lỗi nào được tạo -> Cho phép tạo log đầu tiên
                        $allow_insert = true;
                    } else {
                        // Nếu đã có log trước đó -> Tính khoảng cách thời gian thực tế bằng giây (PHP)
                        $seconds_since_last_warning = time() - strtotime($last_warning['event_time']);
                        
                        // Nếu khoảng cách lớn hơn hoặc bằng 120 giây (đúng 2 phút giãn cách) -> Tiếp tục kích hoạt log mới
                        if ($seconds_since_last_warning >= 120) {
                            $allow_insert = true;
                        }
                    }

                    // Thực thi tạo lịch sử và thông báo lặp lại định kỳ đúng chu kỳ 2 phút
                    if ($allow_insert) {
                        mysqli_query($conn, "INSERT INTO room_logs (room_id, event_time, event_type, details, amount) VALUES ($current_room_id, NOW(), 'LƯU Ý', '$alert_details', 0)");
                        
                        // Duy trì kích hoạt viền đỏ nhấp nháy khẩn cấp trên sơ đồ lễ tân
                        mysqli_query($conn, "UPDATE rooms SET is_forget_warning = 1 WHERE id = $current_room_id");
                        
                        if (function_exists('sendTelegramNotification')) {
                            sendTelegramNotification("🚨 <b>CẢNH BÁO AN NINH ĐỊNH KỲ (MỖI 2 PHÚT):</b>\n🏨 Phòng <b>{$row['room_name']}</b> vẫn đang MỞ CỬA!\n⚠️ Tình trạng: Quá 20 phút check-out nhưng chưa dọn.\n❌ Nhật ký: <i>Chưa bắt đầu dọn vệ sinh nhưng cửa mở bất thường.</i>");
                        }
                    }
                } else {
                    // TRONG VÒNG 20 PHÚT ĐẦU: Giữ im lặng, cho phép ra vào tự do để kiểm tra phòng
                }
            } else {
                // NHÂN VIÊN ĐÃ BẤM BẮT ĐẦU DỌN HỢP LỆ: Cho ra vào thoải mái, chặn spam log mỗi 1 phút
                $notice_details = "🧹 HỆ THỐNG: Cửa mở trong lúc nhân viên đang dọn dẹp vệ sinh hợp lệ.";
                $check_dup_notice = mysqli_query($conn, "SELECT id FROM room_logs WHERE room_id = $current_room_id AND details = '$notice_details' AND event_time > NOW() - INTERVAL 1 MINUTE");
                if (mysqli_num_rows($check_dup_notice) == 0) {

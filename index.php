...
<head>
    ...
    <style>
        ...
        /* Hiệu ứng nhấp nháy khẩn cấp dành riêng cho phòng quên đóng cửa */
        ...
        /* TÁC ĐỘNG CSS KHI BẬT DARK MODE */
...
<body>
    ...
    <div class="log-section">
        ...
        <?php if (isAdmin()): ?>
            <a href="admin_logs.php" class="nav-btn danger">📊 TRANG QUẢN TRỊ CAO CẤP (BÁO CÁO & EXCEL)</a>
        <?php endif; ?>
    </div>
    ...
    // Bộ dao động âm thanh cảnh báo lỗi quên đóng cửa
    function playEmergencySound() {
        ...
        if (audioCtx) {
            ...
            oscillator.type = 'sawtooth'; // Âm thanh dạng còi hú dữ dội hơn dạng sine
            oscillator.frequency.setValueAtTime(988, audioCtx.currentTime); 
            gainNode.gain.setValueAtTime(0.15, audioCtx.currentTime);
            oscillator.start();
            oscillator.stop(audioCtx.currentTime + 0.3); // Hú còi kéo dài 0.3 giây
        }
    }

    // Hàm lưu trạng thái và bật/tắt Dark Mode bằng LocalStorage để khi load trang không bị mất chế độ màu
        ...
        fetch('api_get_status.php')
            ...
            .then(data => {
                ...
                if (currentRoomsState !== lastRoomsState) {
                    ...
                    let roomHtml = '';
                    isWarningActive = false; // Reset trạng thái còi hú trước khi quét
                    ...
                    data.rooms.forEach(room => {
                        ...
                        // TÍNH NĂNG 1: Nếu Server báo cửa mở quá lâu (is_forget_warning = true), chèn class css báo động nhấp nháy
                        let warningClass = room.is_forget_warning ? 'door-warning' : '';
                        if (room.is_forget_warning) {
                            isWarningActive = true; // Kích hoạt còi báo động tổng
                        }
    ...
    // Vòng lặp âm thanh độc lập phát ra mỗi 3 giây nếu còi hú được kích hoạt
    setInterval(() => {
        if (isWarningActive) {
            playEmergencySound();
        }

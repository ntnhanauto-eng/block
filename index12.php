...
<head>
    ...
    <style>
        ...
        /* LƯỚI PHÒNG: MẶC ĐỊNH TRÊN MOBILE LÀ 2 PHÒNG MỘT HÀNG */
        ...
        .room-card p { margin: 6px 0 0 0; font-size: 13px; color: #333; }
        ...
        fetch('api_get_status.php')
            ...
            .then(data => {
                ...
                data.rooms.forEach(r => {
                    ...
                    // Cả 'cho_ve_sinh' và 've_sinh' đều tính vào nhóm đang xử lý/vệ sinh tổng quan
                    else if (r.status === 've_sinh' || r.status === 'cho_ve_sinh') vesinh++;
                });
                ...
                if (forceRender || currentRoomsState !== lastRoomsState) {
                    ...
                    data.rooms.forEach(room => {
                        ...
                        // Xử lý Logic màu sắc và tên hiển thị tự động theo Trạng thái hệ thống
                        if (room.status === 'khach') {
                            cardBgColor = '#fce4d6'; // Màu hồng nhạt khi có khách
                            displayName = 'Có khách ở';
                        } else if (room.status === 'cho_ve_sinh') {
                            cardBgColor = '#fff2cc'; // Màu vàng nhạt khi vừa trả phòng
                            displayName = 'Chờ dọn vệ sinh';
                        } else if (room.status === 've_sinh') {
                            cardBgColor = '#fff2cc'; // Giữ màu vàng nhạt khi đang dọn dẹp
                            displayName = 'Đang dọn vệ sinh';
                        } else {
                            cardBgColor = '#e2f0d9'; // Màu xanh lá cây khi sạch sẽ đón khách
                        ...
                        // Loại bỏ hoàn toàn khối <select> tự chọn thủ công để tránh can thiệp sai quy trình
                        roomHtml += `

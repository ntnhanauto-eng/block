...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'finish_clean') {
    ...
    if ($user_pin !== CLEANING_PIN) {
        $message = "<div class='alert error'>❌ Sai mã PIN bảo mật cho $room_name! Vui lòng thử lại.</div>";
    } else {
        ...
        if ($update) {
            ...
            $message = "<div class='alert success'>🎉 Đã cập nhật $room_name thành phòng trống thành công!</div>";
        } else {
            $message = "<div class='alert error'>Có lỗi kết nối cơ sở dữ liệu!</div>";
        }
    }
}

...
while ($row = mysqli_fetch_assoc($rooms_query)) {
    ...
    if (!empty($token) && isset($devices[$current_room_id])) {
        ...
        // =========================================================================
        // 🔥 ĐÃ CẤU HÌNH TRỌN VẸN: LOGIC PHÂN TẦNG VÀ ĐẾM NHẮC NHỞ ĐỊNH KỲ MỖI 2 PHÚT
        // =========================================================================
        if ($tuya_door_state === 'Mở' && ($row['status'] === 've_sinh' || $row['status'] === 've_sink')) {
            ...
            if (!$has_started_cleaning) {
                ...
                if ($time_passed_minutes > 20) {
                    ...
                    // ⏱️ KIỂM TRA ĐỊNH KỲ 2 PHÚT: Tìm xem trong 2 phút (120s) qua đã tạo bản ghi này chưa
                    ...
                    // Nếu quá 2 phút chưa có log lặp lại -> Ghi tiếp 1 dòng mới và nổ Telegram tiếp

...
if ($room['status'] === 'khach') {
    ...
    if ($log_checkin) {
        ...
        // Cải tiến: Bóc tách Tên, CCCD, Giá phòng và Số người bằng Regex từ chuỗi details mới
    ...
    // Tìm tất cả các dịch vụ phát sinh (Nước suối, thuốc lá...) đã gọi thêm trong ca này
    $services_q = mysqli_query($conn, "SELECT event_time, details FROM room_logs WHERE room_id = $room_id AND event_type = 'DỊCH VỤ' AND event_time >= '{$guest_info['checkin_time']}' ORDER BY id ASC");
    while ($srv = mysqli_fetch_assoc($services_q)) {
        $guest_info['services'][] = $srv;
    }
}

// 3. XỬ LÝ CHECK-IN (ĐÓN KHÁCH VÀO PHÒNG - BỔ SUNG SỐ NGƯỜI)
if (isset($_POST['check_in'])) {
    ...
    // ĐÃ SỬA: Thêm "Số người" và định dạng giá tiền có dấu chấm phân ngàn lưu trực tiếp vào chi tiết sự kiện
...
// 4. XỬ LÝ KHI KHÁCH GỌI THÊM ĐỒ (NƯỚC SUỐI, THUỐC LÁ...)
if (isset($_POST['add_service'])) {
    ...
    // ĐÃ SỬA: Định dạng tiền tệ có dấu phân ngàn hiển thị thẳng trong nhật ký
    $formatted_srv_price = number_format($service_price, 0, ',', '.');
    $details_srv = "Khách gọi thêm: $service_name - Tiền dịch vụ: [{$service_price}] ({$formatted_srv_price}đ)";
    
    mysqli_query($conn, "INSERT INTO room_logs (room_id, event_time, event_type, details) VALUES ($room_id, '$time_now', 'DỊCH VỤ', '$details_srv')");
    
    if (function_exists('sendTelegramNotification')) {
        sendTelegramNotification("🛒 <b>DỊCH VỤ PHÁT SINH:</b>\n🏨 <b>{$room['room_name']}</b>\n📦 Đồ gọi thêm: $service_name\n💰 Số tiền: {$formatted_srv_price}đ");
    }

    header("Location: booking.php?room_id=$room_id&open_panel=1"); // Giữ trạng thái mở bảng sau khi thêm đồ
    exit();
}

// 5. XỬ LÝ CHECK-OUT (TỰ ĐỘNG TÍNH TOÁN)
if (isset($_POST['check_out'])) {
    ...
    foreach ($guest_info['services'] as $srv) {
        if (preg_match('/\[(\d+)\]/', $srv['details'], $srv_matches)) {
            $total_services_bill += (int)$srv_matches[1];
        }
    }

    $total_bill = $room_bill + $total_services_bill;

    mysqli_query($conn, "UPDATE rooms SET status = 've_sink' WHERE id = $room_id");
    mysqli_query($conn, "UPDATE rooms SET status = 've_sinh' WHERE id = $room_id"); // Chống bẫy lỗi gõ chữ
...
<head>
    ...
    <style>
        ...
        /* HIỆU ỨNG TRƯỢT XUỐNG CHO CƠ CHẾ SLIDE DOWN PANEL */
        .slide-panel { display: none; overflow: hidden; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; padding: 15px; margin-top: 15px; box-shadow: inset 0 2px 4px rgba(0,0,0,0.02); }
...
<div class="container">
    ...
    <div class="box">
        ...
        <?php else: ?>
            <button class="btn-toggle" onclick="toggleSlidePanel()">📋 XEM THÔNG TIN LƯU TRÚ & ORDER (BẤM ĐỂ MỞ/ĐÓNG)</button>
                    ...
                    <tbody>
                        ...
                        if (count($guest_info['services']) > 0): 
                            foreach ($guest_info['services'] as $srv):
                                ...
                                if (preg_match('/\[(\d+)\]/', $srv['details'], $p_matches)) { $s_price = (int)$p_matches[1]; }
                        ...
                        <?php endforeach; else: ?>
                            <tr><td colspan="2" style="text-align: center; color:#94a3b8; font-style:italic;">Khách chưa gọi thêm đồ uống hay thuốc lá.</td></tr>
                        <?php endif; ?>
            ...
            <fieldset style="border: 1px solid #e67e22; border-radius: 6px; padding: 12px; margin-top: 15px;">
                ...
                <form method="POST" onsubmit="cleanPriceBeforeSubmit('srv_price_input')">
                    <div class="form-group">
                        <input type="text" name="service_name" placeholder="Ví dụ: 2 chai nước suối, 1 bao thuốc" required>
                    </div>
                    <div class="form-group">
...
// Hàm JavaScript điều khiển bảng trượt lên/xuống (Slide Toggle) nhẹ nhàng
...
// Cơ chế giữ bảng luôn mở nếu lễ tân vừa thực hiện thao tác Thêm món đồ

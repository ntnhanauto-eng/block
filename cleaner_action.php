...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['start_cleaning'])) {
        // KIỂM TRA CHẶN: Chỉ ghi log khi chưa từng tồn tại mốc bắt đầu trong chu kỳ này
        ...
        $message = "<div class='alert success'>🔓 ĐÃ XÁC NHẬN! Hệ thống đồng bộ tiến độ dọn phòng thành công.</div>";
    }
...
// QUÉT THỜI GIAN ĐỒNG BỘ CHO ĐỒNG HỒ REAL-TIME
$has_started_cleaning = false;
$start_time_string = "";
$seconds_elapsed = 0;
$cleaner_name = $current_user;

if ($room['status'] === 've_shin' || $room['status'] === 've_sinh') {
...
<div class="card">
    ...
    <form method="POST">
        <?php if ($room['status'] === 've_sinh'): ?>
            ...
            <?php endif; ?>
        <?php else: ?>
            <p style="color: #64748b; font-size: 14px; font-style: italic;">Phòng đã sạch sẽ hoặc có khách.</p>
        <?php endif; ?>
    </form>

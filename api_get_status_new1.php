    ...
    if (!empty($token) && isset($devices[$current_room_id])) {
        ...
        if ($tuya_door_state !== $previous_state) {
            ...
            if ($tuya_door_state === 'Mở' && ($row['status'] === 've_sinh' || $row['status'] === 've_sink')) {
                ...
                $checkout_time = $checkout_log['event_time'] ?? date('Y-m-d H:i:s');
                
                // Kiểm tra xem Cleaner đã bấm "BẮT ĐẦU DỌN PHÒNG" thông qua mã QR hay trang tổng chưa (Sửa điều kiện LIKE chuẩn hóa tên nhân viên)
                $cleaner_start_q = mysqli_query($conn, "SELECT id FROM room_logs WHERE room_id = $current_room_id AND details LIKE 'BẮT ĐẦU DỌN PHÒNG%' AND event_time > '$checkout_time'");

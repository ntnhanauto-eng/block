...
while ($row = mysqli_fetch_assoc($rooms_query)) {
    ...
    if (!empty($token) && isset($devices[$current_room_id])) {
        ...
        if ($tuya_door_state !== $previous_state) {
           ...
           // =========================================================================
            // 🔥 ĐÃ SỬA HOÀN CHỈNH: LOGIC PHÂN TẦNG 20 PHÚT AN NINH PHÒNG CHỜ VỆ SINH
            // =========================================================================
            if ($tuya_door_state === 'Mở' && ($row['status'] === 've_sinh' || $row['status'] === 've_sink')) {
                
                // THUẬT TOÁN MỚI: Dò thẳng bản ghi Lễ tân/Hệ thống đổi trạng thái sang ve_sinh gần đây nhất
                ...
                // Nếu xui rủi không tìm thấy log nào, ép mốc thời gian lùi lại hẳn 1 tiếng trước để kích hoạt chế độ bảo vệ ngay lập tức
                $checkout_time = $checkout_log['event_time'] ?? date('Y-m-d H:i:s', strtotime('-1 hour'));
                
                // Kiểm tra xem Cleaner đã bấm "BẮT ĐẦU DỌN PHÒNG" thông qua mã QR hay trang tổng chưa
                ...
                if (!$has_started_cleaning) {
                    // Nếu CHƯA bấm bắt đầu dọn -> Tính số phút kể từ lúc Lễ tân Check-out
                    $time_passed_minutes = (time() - strtotime($checkout_time)) / 60;

                    if ($time_passed_minutes > 20) {
                        // ❌ ĐÃ QUÁ 20 PHÚT: Báo động lập tức, tạo chữ LƯU Ý vào Database đúng yêu cầu
                        $alert_details = "Chưa bắt đầu dọn vệ sinh nhưng cửa mở bất thường";
                        
                        // Chặn trùng lặp: Nếu trong vòng 30 giây qua đã ghi nhận log này rồi thì không ghi đè tiếp tránh tràn dữ liệu
                        $check_duplicate = mysqli_query($conn, "SELECT id FROM room_logs WHERE room_id = $current_room_id AND details = '$alert_details' AND event_time > NOW() - INTERVAL 30 SECOND");
                        ...
                        if (mysqli_num_rows($check_duplicate) == 0) {
                            ...
                            // Kích hoạt chớp viền cảnh báo màu đỏ khẩn cấp trên trang index.php
                            mysqli_query($conn, "UPDATE rooms SET is_forget_warning = 1 WHERE id = $current_room_id");
                            
                            if (function_exists('sendTelegramNotification')) {
                                sendTelegramNotification("🚨 <b>CẬP NHẬT AN NINH XÂM NHẬP:</b>\n🏨 Phòng <b>{$row['room_name']}</b> đột ngột bị MỞ CỬA!\n⚠️ Trạng thái: Chờ dọn dẹp đã quá 20 phút.\n❌ Nhật ký: <i>Chưa bắt đầu dọn vệ sinh nhưng cửa mở bất thường.</i>");
                            }
                        }
                    } else {
                        // 🟢 TRONG VÒNG 20 PHÚT ĐẦU: Để mở cửa tự do tự nhiên, không ghi log cảnh báo gì cả.
                    }
                } else {
                    // 🟢 NHÂN VIÊN ĐÃ BẤM BẮT ĐẦU DỌN PHÒNG: Cho phép ra vào cửa thoải mái, chỉ ghi nhận nhật ký thông thường
                    $notice_details = "🧹 HỆ THỐNG: Cửa mở trong lúc nhân viên đang dọn dẹp vệ sinh hợp lệ.";
                    
                    // Chặn trùng log thông thường mỗi 1 phút

...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    // Đã thêm l.amount vào câu lệnh SELECT
    ...
    while ($row = mysqli_fetch_assoc($export_query)) {
        $money = (int)$row['amount']; // Đọc trực tiếp số nguyên
        ...
        echo "<td style='text-align:right; font-weight:bold;'>{$money}</td>"; // Dạng số thuần túy để Excel tự tính hàm SUM
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="color: #28a745;">Số Tiền Thu (đ)</th>
            ...
            <?php if (mysqli_num_rows($all_logs) > 0): ?>
                <?php while($l = mysqli_fetch_assoc($all_logs)): ?>
                    <?php 
                        ...
                        // ĐÃ SỬA: Lấy trực tiếp từ giá trị cột l.amount trong cơ sở dữ liệu
                    ...
                    <tr class="<?php echo $row_class; ?>">
                        ...
                        <td style="font-weight: bold; text-align: right; color: <?php echo $money_display > 0 ? '#28a745' : '#666'; ?>;">
